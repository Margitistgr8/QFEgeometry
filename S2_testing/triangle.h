...
Eigen::VectorXd TotalAreaList(Lattice &lattice, int L)
{

  Tensor2D<int> TriangleList = TriangleVertexMapping(lattice.Vertices, L); 
  Eigen::VectorXd Area(TriangleList.size()); 
  for (int n = 0; n<TriangleList.size(); n++)
  {
    Eigen::Vector3d r1 = lattice.rvec[TriangleList[n][0]];
    Eigen::Vector3d r2 = lattice.rvec[TriangleList[n][1]];
    Eigen::Vector3d r3 = lattice.rvec[TriangleList[n][2]];
    Area[n] = TriangleArea(r1, r2, r3);
  }
  return Area; 
}
...
Eigen::VectorXd returnCurrentArea(Lattice &lattice, std::vector<Triangle> &TList)
{
...
// Tensor2D<int> FindOrbitRepresentative(Tensor2D<int> &Vertices){
//   std::vector<int> OrbitLabels = FindOrbitLabels(Vertices); 
...
//     std::vector<int> pos = {Vertices[index][0],Vertices[index][1],Vertices[index][2]};
//     std::sort(pos.begin(), pos.end()); 
...
// Tensor2D<int> FindBasis(Tensor2D<int> &Vertices)
// {
//   Tensor2D<int> results; 
//   Tensor2D<int> OrbitReps = FindOrbitRepresentative(Vertices);
//   int dof = 0; 
//   for (int i=0; i<OrbitReps.size(); i++)
//   {
