...
//Lattice structure that tessellates an equilateral face of the platonic solid
struct Lattice{
  ...
  std::vector<int> VertexLabels;       //size = L given by FindOrbitLabels (orbit of each vertex)
  //The following three set by calling "InitializeOrbitInformation"
  std::vector<int> OrbitPos;           //size = Number of Orbits (give the vertex label of orbit representative for each orbit)(orbits refer to cosets under the symmetry group of the equilateral traingle)
  Tensor2D<int> Basis;                 //size = (Number of Orbits, 2) (for each vertex, where it is a dof and # of direction it can move in)
...
//Given the list of vertices L={{x1, y1, z1}, ...}
//returns a list of all triangles on the lattice with its vertices 
//vertices are stored by their order in L i = {xi, yi, zi}
//each traingle is stored as {i,j,k} where i indexes the position of the vertex in L
...
//Given the barycentric coordinate on the lattice, the rotations and reflections of the base equilateral traingle 
//is realized by identifying all points {xi, yi,zi} with any permutation in order
//returns a list of the label (orbit index) corresponding to each vertex. 
std::vector<int> FindOrbitLabels(Tensor2D<int> &Vertices){
  ...
  for(int n = 0; n< Vertices.size(); n++){
      ...
      //Identified the first instance of an orbit
      else{
        classlabels[n]=dof_counter;
        counter[n] = 1; 
        int ncounted = 1; 
        std::vector<int> xi= {Vertices[n][0],Vertices[n][1],Vertices[n][2]};
        std::sort(xi.begin(), xi.end()); //Using as orbit representative the permutation of increasing magnitude

        //iterate through unlabeled points to find orbit elements by matching 
        for (int n1=0; n1< Vertices.size(); n1++){
            if (ncounted==24){break;} //Number of 24 confuses me, since 3!=6? I am not changing it now because maybe I am mistaken at this moment
...
void InitializeOrbitInformation(Lattice &lattice)
{
  ...
  //Finding the position in Vertices of each orbit representative (for class i, where is the vertex that represents it)
  //Orbit rep is conventionally choosen to be the sorted barycentric coordinates (in increasing order)
  ...
  for (int o=0; o<n_orbit; o++){
    auto it = std::find(lattice.VertexLabels.begin(), lattice.VertexLabels.end(), o); //Find first instance of the orbit in the list
    ...
    auto it2 = std::find(lattice.Vertices.begin(), lattice.Vertices.end(), pos); //Find the instance of the orbit which is the orbit rep
    OrbitPos.push_back(std::distance(lattice.Vertices.begin(), it2));
  }
  lattice.OrbitPos = OrbitPos; 

  //Finding the basis (For each vertex, (a,b), a= -1 (not a dof) or dof, b=number of independent direction wrt symmetry constraint)
  Tensor2D<int> results; //Basis 
  int dof = 0; //Total number of points on the lattice that can be moved
  for (int i=0; i<n_orbit; i++)
  {
    ...
    //Finding how many zeros are in the orbit rep since zeros give information on whether the degeneracy is less than six
    int distance = std::distance(vec.begin(), it);
    if (distance!=0)
    {
      if (distance==2){results.push_back({-1, 0});}//This is the vertex on the equilateral vertices, fixed by symmetry
      else
      {  
      std::set<int> unique_labels(it, vec.end());
      int n_distinct = unique_labels.size(); 
      if (n_distinct==1){results.push_back({-1, 0});}//These are the midpoints on the equilateral edges, fixed by symmetry
      else
      {
        results.push_back({dof, 1}); //These are points along the edge (w/a midpoints), they can move in one direction (contraint to lie on the edge)
        dof+=1; 
      }

      }
    }
    //Even though we have removed some points that are constrained, even in the bulk
    //there are still contraints for points along the diagonal. The center point in the lattice is fixed by symmetry
    else
    {
...
//Saving the dual area and deficit angle and effectve lattice spacing at each site 
void SaveVertexGeometricInformation(Lattice &lattice, FILE* file, Tensor2D<int> &TList, int L)
{
  Eigen::VectorXd def=returnCurrentDeficit(lattice, TList, L);
  Eigen::VectorXd dual=returnCurrentDualArea(lattice, TList, L);

  for (int i=0; i<lattice.Vertices.size();i++)
  {
   fprintf(file, "%d %.16f %.16f %.16f %.16f %.16f\n", i, lattice.rvec[i][0], lattice.rvec[i][1], lattice.rvec[i][2], def[i], dual[i]);
  }
}
