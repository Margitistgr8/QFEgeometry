...
void PrintClusterDetails(ContinuousTimeLattice& lattice, double invT) {
    std::unordered_map<Node*, std::vector<std::tuple<int, double, double, int>>> clusterMap;

    for (int site = 0; site < lattice.SpatialLattice.n_sites; ++site) {
        const CircularLinkedList& lad = lattice.TimeLadders[site];
        Node* temp = lad.head;
        if (!temp) continue;

        do {
            int rootID = lattice.uf.find(temp);
            Node* root = lattice.uf.idToNode[rootID];

            double start = temp->data;
            double end = temp->next->data;
            clusterMap[root].emplace_back(site, start, end, temp->spin);
    ...
    for (auto it = clusterMap.begin(); it != clusterMap.end(); ++it) {
        Node* root = it->first;
        const std::vector<std::tuple<int, double, double, int>>& segments = it->second;

        std::cout << "Cluster #" << cluster_id++ << ":\n";
        for (size_t i = 0; i < segments.size(); ++i) {
            int site = std::get<0>(segments[i]);
            double start = std::get<1>(segments[i]);
            double end = std::get<2>(segments[i]);
            int spin = std::get<3>(segments[i]);
...
std::cout << "  Seed: " << seed << ", Iterations: " << n_iter 
    ...
    for (int step = 0; step < n_iter + n_therm; ++step) 
    {
        ...
        int cluster_num_pre = Ising.CountClusters();
        
        // printf("cluster number before update is: %d\n", cluster_num_pre);
        // printf("Printing data from BuildRung!!\n");
        res = Ising.BuildRungBug(thread_rngs, 0);
       
        int cluster_num = Ising.CountClusters();
        //printf("cluster number after update is: %d\n", cluster_num);
        //PrintClusterDetails(field, invT);
